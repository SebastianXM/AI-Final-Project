...
class NeuralNetwork:
    ...
    def back_propogation(self, x, y):
        if self.output_size != 1:
            y_hat = np.zeros(self.output_size)
            y_hat[y] = 1
        else:
            y_hat = y
...
if __name__ == "__main__":
    ...
    for percentage in percentages:
        ...
        for run in range(num_runs):
            ...
            face_nn = NeuralNetwork(60*70, 100, 100, 1)
            
            digits_total_time = digits_nn.train(digits_X_train_subset, digits_y_train_subset, 1000, 0.001, 0.0)
            face_total_time = face_nn.train(face_X_train_subset, face_y_train_subset, 1000, 0.001, 0)

            # store_weights(0, digits_nn, percentage) # to store the weights (not really used right now)
            # store_weights(1, face_nn, percentage) # to store the weights (not really used right now)

            digits_test_accuracy = digits_nn.test(digits_X_train_subset, digits_y_train_subset)
            face_test_accuracy = face_nn.test(face_X_train_subset, face_y_train_subset)
